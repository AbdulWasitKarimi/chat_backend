...
const createToken = (_id) => {
    ...
    return jwt.sign({_id}, jwtkey,{expiresIn:"1000d"})
}
...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (!validator.isStrongPassword(password)) return res.status(400).json("You must choose a strong password! ")
...
const loginUser = async(req, res) => {
    ...
    try {
      ...
      if (!isValdPassword) return res.status(400).json("Invalid email or password! ")
